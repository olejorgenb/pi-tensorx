...
Fetches have a per-attempt timeout and retry transient failures (network errors, timeouts, HTTP 429/5xx, invalid responses) with backoff. At load the extension registers whatever it can reach on its own: the live catalog if `TENSORX_API_KEY` is in the environment, the last persisted catalog if there is one, and the bundled snapshot otherwise. At that point the extension is being constructed and has no session context to resolve a saved credential from, so the environment is the only key source available. That startup fetch runs on a reduced timeout and retry budget, since pi waits for extensions to finish loading. The snapshot is also what lets TensorX appear under `/login` → API Keys, since pi only lists providers that have registered models.
