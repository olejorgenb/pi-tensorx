...
The backends behind TensorX do not accept the same `reasoning_effort` values, and the catalog says nothing about which ones they take. `qwen/qwen3.8-*` accepts only `low`, `medium`, and `xhigh` (its default); `z-ai/glm-5.1` rejects `minimal`; every other model takes pi's full range. Without that knowledge, picking `high` on a qwen3.8 model failed the request with `400 Unexpected reasoning effort high`, and `xhigh` was not offered at all, because pi only shows `xhigh` for a model that maps it.

The extension therefore registers a `thinkingLevelMap` for the models that need one, so `/thinking` lists what the model accepts and a level saved from another model is clamped to the nearest supported one instead of erroring. `off` is also honest now: it disables thinking on `qwen/qwen3.8-27b` and `qwen/qwen3.8-flash-next` (before, it sent nothing and the model kept reasoning at its default) and is hidden on `qwen/qwen3.8-2.4t-a95b`, which rejects disabling thinking.

These values were found by probing the API, not read from the catalog, so they can go stale — a model that learns a new level is one this extension will not offer until the table is updated. Everything not listed in `MODEL_THINKING_LEVELS` keeps pi's defaults.
