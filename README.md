...
A key saved through `/login` is enough to keep the catalog current: pi passes it to the extension's `refreshModels()` hook when it refreshes model lists. `TENSORX_API_KEY` still works and is still what lets the catalog load during the very first startup, before any key is saved.
...
At load the extension registers whatever it can reach without help from pi: the live catalog if `TENSORX_API_KEY` is in the environment, the bundled snapshot otherwise. The snapshot is what lets TensorX appear under `/login` → API Keys, since pi only lists providers that have registered models.
