...
`MODEL_THINKING_LEVELS` maps pi thinking levels to the `reasoning_effort` values a model accepts. pi derives the levels it offers from `getSupportedThinkingLevels()` (`packages/ai/src/models.ts`): with no `thinkingLevelMap` a reasoning model gets `off`/`minimal`/`low`/`medium`/`high`, `xhigh` and `max` appear only when the map has a (non-null) entry for them, and a `null` entry removes a level. `clampThinkingLevel()` then rewrites a requested level that isn't available to the nearest one that is, scanning up first — which is what keeps a setting carried over from another model from failing the request.

The backends disagree, and only the request tells you: `qwen/qwen3.8-*` accepts `low`, `medium`, `xhigh` and answers anything else with `400 Unexpected reasoning effort <x>. Supported types are xhigh (default), medium, and low.`; `qwen/qwen3.8-2.4t-a95b` additionally rejects `"none"` with `Disabling thinking is not supported`; `z-ai/glm-5.1` rejects `minimal` and `xhigh`. Everything else in the catalog accepted `minimal` through `xhigh` when probed. Note that "accepted" only means no 400 — whether a backend honors `xhigh` is not observable this way, which is why models that merely tolerate a level are left on pi's defaults rather than being given an `xhigh` mapping.

The table is hardcoded because the catalog carries nothing to derive it from: `model_info` has `supports_reasoning` and nothing else about reasoning, `supported_openai_params` does not even list `reasoning_effort` for the qwen3.8 models that clearly accept it, and duplicate entries for the same `model_name` disagree with each other. The 400 text is upstream passthrough, so a backend can change its accepted values without the catalog changing at all.

This is deliberately not the mistake `FALLBACK_CATALOG` was. That snapshot asserted which models exist and what they cost, and drifted silently into wrong prices. This table asserts nothing about the catalog: entries are matched by lowercased ID prefix against whatever the live catalog returns, a renamed model simply stops matching and reverts to pi's defaults, and the worst drift is offering one level too few or too many for a model that changed. `off: "none"` on qwen3.8 is the one entry probed only as "accepted", not verified to disable thinking — the alternative was the status quo, where `off` sent no field at all and the model reasoned at its `xhigh` default.

`fromStoredModel()` re-derives the map from the model ID instead of reading it back from `models-store.json`, so a catalog persisted by an older version of the table picks up the current one.
