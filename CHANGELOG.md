...
- Offers the thinking levels each model actually accepts. TensorX backends disagree about `reasoning_effort`: the `qwen/qwen3.8-*` models take only `low`, `medium`, and `xhigh` (and default to `xhigh`), while `z-ai/glm-5.1` rejects `minimal`. Picking a level a model rejects failed the request with a raw `400 Unexpected reasoning effort …`, and `xhigh` could not be selected at all because pi only offers it when a model maps it. Those models now register a `thinkingLevelMap`, so `/thinking` lists the supported levels, `xhigh` is selectable on qwen3.8, and a saved-but-unsupported level is clamped to the nearest one that works instead of erroring. This also makes `off` real on `qwen/qwen3.8-27b` and `qwen/qwen3.8-flash-next` — it previously sent nothing and left the model reasoning at its default — and hides `off` on `qwen/qwen3.8-2.4t-a95b`, which cannot disable thinking.
